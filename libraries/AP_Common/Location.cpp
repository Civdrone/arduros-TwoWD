...
Vector2f Location::get_distance_NE(const Location &loc2) const
{
    ...
    //GPS reading
    loc1_lat_hp = (double)lat + (lat_hp / 10.0f);
    loc1_lng_hp = (double)lng + (lng_hp / 10.0f);
    
    //Destination
    loc2_lat_hp = (double)loc2.lat + (loc2.lat_hp / 10.0f);
    loc2_lng_hp = (double)loc2.lng + (loc2.lng_hp / 10.0f);
...
void Location::offset(float ofs_north, float ofs_east)
{
    ...
    double hplat = (double) lat + (lat_hp / 10.0f); //(lat_hp / 100.0f);
    double hplng = (double)lng + (lng_hp / 10.0f); //(lng_hp / 100.0f);
    ...
    lat = (int32_t)intpart;
    lat_hp = (int8_t)(fractpart * 10.0f);

    fractpart = modf(hplng, &intpart);
    lng = (int32_t)intpart;
    lng_hp = (int8_t)(fractpart * 10.0f);
